...
const MovieProvider = (props) => {
  ...
  const searchMovies = async (query) => {
    try {
      ...
      if (result) {
        ...
        const { data } = await axios.post(`${server}/api/movies/search`, { query });
  ...
  const getMovieById = async (movieId) => {
    try {
      ...
      if (movie) {
        ...
        const { data } = await axios.get(
          `${server}/api/movies/movie/${movieId}`,
          token
            ? {
                headers: {
                  Authorization: `Bearer ${token}`,
  ...
  const getRecommended = async () => {
    try {
      ...
      if (token && !homeMovies.recommended.length && state.history.length >= 5) {
        const { data } = await axios.get(`${server}/api/movies/recommended`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const getHistory = async () => {
    try {
      ...
      if (token && historyMovies.length === 0) {
        const { data } = await axios.get(`${server}/api/movies/history`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  return <MovieContext.Provider value={values}>{props.children}</MovieContext.Provider>;
};
