...
const Profile = () => {
  ...
  const validate = () => {
    ...
    return !errors.currPass && !errors.newPass && !errors.confirmPass ? true : false;
  };
  ...
  const handleChange = (event) => {
    ...
    if (form.newPass !== form.confirmPass && event.target.name === "confirmPass") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isValid) {
        await axios.put(`${server}/api/auth/change-password`, form, {
          headers: {
        ...
        <div className={classes.ProfileContent}>
          ...
          <div className={classes.Password}>
            ...
            <form className={classes.PasswordForm} onSubmit={handleSubmit}>
              ...
              />
              {validation.currPass && <span className="text-danger pb-3">{validation.currPass}</span>}
              <input
                type="password"
                name="newPass"
                value={form.newPass}
              ...
              />
              {validation.newPass && <span className="text-danger pb-3">{validation.newPass}</span>}
              <input
                type="password"
                name="confirmPass"
              ...
              />
              {validation.confirmPass && <span className="text-danger pb-3">{validation.confirmPass}</span>}
